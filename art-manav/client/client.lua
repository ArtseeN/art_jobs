...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('art_manav:server:getConfig', function(sConfig)
        Jobs = sConfig
    end)
                    ...
                    for i=1, #v.location do
                        ...
                        if show then
                            ...
                            if IsControlJustPressed(0, 38) and distance <= v.location[i].draw3dtext["distance"] then
                                ESX.TriggerServerCallback('art_manav:checkCount', function(count)
                                    if v.location[i].item["process"] == "pdomates" or count > v.location[i].item["removeCount"]-1 then
                                        ...
                                        if v.location[i].progressbar["enable"] then
                                            TriggerEvent("mythic_progbar:client:progress", {
                                                ...
                                                if not status then
                                                    TriggerServerEvent("art_manav:server:process", coords, k, v, i)
                                                else
                                            ...
                                            Citizen.CreateThread(function()
                                                ...
                                                ClearPedTasksImmediately(ped)
                                                TriggerServerEvent("art_manav:server:process", v.location[i].item)
                                            end)
                                        end
                                    else
