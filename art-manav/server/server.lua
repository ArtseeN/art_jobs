...
ESX.RegisterServerCallback('art_manav:server:getConfig', function(source, cb, itemName) 
    while Jobs == nil do 
        Citizen.Wait(100)
    end
    cb(Jobs)
...
RegisterServerEvent("art_manav:server:process")
AddEventHandler("art_manav:server:process", function(coords, k, v, id)
    ...
    if #(jobData.coords - coords) <= 10 then
        ...
        if item or jobData.item["process"] == "sell" then
            ...
            if jobData.item["process"] ~= "sell" and WeightSystem then
                if jobData.item["process"] == "tdomates" or jobData.item["process"] == "tdomates" then
            ...
            if jobData.item["process"] == "qdomates" then
                if retval then
            ...
            elseif jobData.item["process"] == "pdomates" then
            ...
            elseif jobData.item["process"] == "tdomates" then
                if requiredItem.count >= jobData.item["removeCount"] then
                    if retval then
                        player.removeInventoryItem(jobData.item["requiredItem"], jobData.item["removeCount"])
                        player.addInventoryItem(jobData.item["itemName"], jobData.item["addCount"])
            ...
            print("[art_manav] - ERROR - art_manav:server:process Tried to query an item that isn't included in the 'items' table.") 
            print("[art_manav] Item must be "..jobData.item["itemName"])
        end
    else
        print("[art_manav] - WARNING - art_manav:server:process event called without close to the process coords. User Identifier:"..player.identifier)
        DropPlayer(player.source, "Kicked for using exploit!")
    end
end)
ESX.RegisterServerCallback('art_manav:checkCount', function(source, cb, itemName) 
    local player = ESX.GetPlayerFromId(source)
