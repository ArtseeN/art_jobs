...
ESX.RegisterServerCallback('art_odun:server:getConfig', function(source, cb, itemName) 
    while Jobs == nil do 
        Citizen.Wait(100)
    end
    cb(Jobs)
...
RegisterServerEvent("art_odun:server:process")
AddEventHandler("art_odun:server:process", function(coords, k, v, id)
    ...
    if #(jobData.coords - coords) <= 10 then
        ...
        if item or jobData.item["process"] == "sell" then
            ...
            print("[art_odun] - ERROR - art_odun:server:process Tried to query an item that isn't included in the 'items' table.") 
            print("[art_odun] Item must be "..jobData.item["itemName"])
        end
    else
        print("[art_odun] - WARNING - art_odun:server:process event called without close to the process coords. User Identifier:"..player.identifier)
        DropPlayer(player.source, "Kicked for using exploit!")
    end
end)
ESX.RegisterServerCallback('art_odun:checkCount', function(source, cb, itemName) 
    local player = ESX.GetPlayerFromId(source)
